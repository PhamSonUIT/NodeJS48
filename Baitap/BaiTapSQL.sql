...
CREATE table like_res(
	like_res_id int PRIMARY KEY NOT NULL AUTO_INCREMENT,
	user_id int,
...
VALUES
    ...
    ('Falafel Wrap', 8.49, 'falafel_wrap.jpg', 'Middle Eastern wrap with falafel, hummus, and veggies.', 18),
...
INSERT INTO orders (user_id, food_id, amount, _code, arr_sub_id)
VALUES
...
SELECT
	count(u.user_id) AS 'số like',
	u.user_id
FROM
	users AS u
	JOIN like_res AS lr ON lr.res_id = u.user_id
GROUP BY
	u.user_id
ORDER BY
...
GROUP BY
	res.res_id
ORDER BY
	COUNT(res.res_id) DESC
LIMIT
	2
SELECT
	COUNT(u.user_id) AS 'lượt order',
	u.full_name
FROM
	users AS u

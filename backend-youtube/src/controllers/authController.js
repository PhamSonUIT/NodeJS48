...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (codeForgotPassExists) {
      ...
      sendMailForgotPassword(res, transporter, mailForgotPass);
    } else {
      let expired = new Date(new Date().getTime() + 2 * 60 * 60 * 1000); // expired sau 2h
...
const resetPassword = async (req, res) => {
  ...
  // code có tồn tại trong db không
  console.log("test 1");
  ...
  // dùng userExists.user_id để update password
  // userExists.pass_word = hashNewPassword;
  // await userExists.save();

  // dùng model forgot_password_code để update
